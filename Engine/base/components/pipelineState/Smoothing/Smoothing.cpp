...
void Smoothing::ShaderCompile() {
	...
	pixelShaderBlob = ShaderCompiler::GetInstance()->CompileShader(L"project/gamedata/resources/shaders/BoxFilter.PS.hlsl", L"ps_6_0");
	assert(pixelShaderBlob != nullptr);
}

void Smoothing::CreateRootSignature() {
...
void Smoothing::CreateInputLayOut() {
	inputLayoutDesc.pInputElementDescs = nullptr;
	inputLayoutDesc.NumElements = 0;
}

void Smoothing::CreateBlendState() {
	//すべての色要素を書き込む
	//何もなし
...
void Smoothing::CreateRasterizarState() {
	//裏面（時計回り）を表示しない
...
void Smoothing::CreatePipelineStateObject() {
