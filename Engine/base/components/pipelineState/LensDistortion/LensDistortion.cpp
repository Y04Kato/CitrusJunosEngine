...
void LensDistortion::ShaderCompile() {
	...
	pixelShaderBlob = ShaderCompiler::GetInstance()->CompileShader(L"project/gamedata/resources/shaders/LensDistortion.PS.hlsl", L"ps_6_0");
	assert(pixelShaderBlob != nullptr);
}

void LensDistortion::CreateRootSignature() {
...
void LensDistortion::CreateInputLayOut() {
	inputLayoutDesc.pInputElementDescs = nullptr;
	inputLayoutDesc.NumElements = 0;
}

void LensDistortion::CreateBlendState() {
	//すべての色要素を書き込む
	//何もなし
...
void LensDistortion::CreateRasterizarState() {
	//裏面（時計回り）を表示しない
...
void LensDistortion::CreatePipelineStateObject() {
