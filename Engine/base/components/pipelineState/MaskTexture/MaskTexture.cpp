...
void MaskTexture::ShaderCompile() {
	...
	pixelShaderBlob = ShaderCompiler::GetInstance()->CompileShader(L"project/gamedata/resources/shaders/MaskTexture.PS.hlsl", L"ps_6_0");
	assert(pixelShaderBlob != nullptr);
}

void MaskTexture::CreateRootSignature() {
	...
	rootParameters[1].DescriptorTable.pDescriptorRanges = descriptorRange2;//tableの中身の配列を指定
	rootParameters[1].DescriptorTable.NumDescriptorRanges = _countof(descriptorRange2);//Tableで利用する数

	//MaskThreshold
	rootParameters[2].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;//CBVを使う
	rootParameters[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;//PixelShaderで使う
	rootParameters[2].Descriptor.ShaderRegister = 2;//レジスタ番号2とバインド
...
void MaskTexture::CreateInputLayOut() {
	inputLayoutDesc.pInputElementDescs = nullptr;
	inputLayoutDesc.NumElements = 0;
}

void MaskTexture::CreateBlendState() {
	//すべての色要素を書き込む
	//何もなし
...
void MaskTexture::CreateRasterizarState() {
	//裏面（時計回り）を表示しない
...
void MaskTexture::CreatePipelineStateObject() {
