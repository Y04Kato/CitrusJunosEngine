...
void Random::ShaderCompile() {
	...
	pixelShaderBlob = ShaderCompiler::GetInstance()->CompileShader(L"project/gamedata/resources/shaders/Random.PS.hlsl", L"ps_6_0");
	assert(pixelShaderBlob != nullptr);
}

void Random::CreateRootSignature() {
...
void Random::CreateInputLayOut() {
	inputLayoutDesc.pInputElementDescs = nullptr;
	inputLayoutDesc.NumElements = 0;
}

void Random::CreateBlendState() {
	//すべての色要素を書き込む
	//何もなし
...
void Random::CreateRasterizarState() {
	//裏面（時計回り）を表示しない
...
void Random::CreatePipelineStateObject() {
