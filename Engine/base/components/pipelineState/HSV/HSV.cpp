...
void HSV::ShaderCompile() {
	...
	pixelShaderBlob = ShaderCompiler::GetInstance()->CompileShader(L"project/gamedata/resources/shaders/HSV.PS.hlsl", L"ps_6_0");
	assert(pixelShaderBlob != nullptr);
}

void HSV::CreateRootSignature() {
...
void HSV::CreateInputLayOut() {
	inputLayoutDesc.pInputElementDescs = nullptr;
	inputLayoutDesc.NumElements = 0;
}

void HSV::CreateBlendState() {
	//すべての色要素を書き込む
	//何もなし
...
void HSV::CreateRasterizarState() {
	//裏面（時計回り）を表示しない
...
void HSV::CreatePipelineStateObject() {
