...
void CreateHoudini::Loadhda(const char* filePath) {
	//開くhdaファイル
	hdaFile_ = filePath;

	//.otlファイルからHoudiniアセットライブラリを読み込む
	HAPI_LoadAssetLibraryFromFile(&session_, hdaFile_, true, &assetLibId_);
	int assetCount;
	//アセットライブラリ内のアセット数の取得
	HAPI_GetAvailableAssetCount(&session_, assetLibId_, &assetCount);
	if (assetCount > 1) {
	...
	HAPI_GetAvailableAssets(&session_, assetLibId_, &assetSh, assetCount);
	std::string assetName = getString(assetSh);
	HAPI_NodeId nodeId;
	HAPI_CreateNode(&session_, -1, assetName.c_str(), "TestObject", false, &nodeId);
	HAPI_CookNode(&session_, nodeId, &cookOptions_);
	int cookStatus;
	...
	HAPI_GetAssetInfo(&session_, nodeId, &assetInfo);
