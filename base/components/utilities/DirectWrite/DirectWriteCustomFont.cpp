...
// 初期化処理
HRESULT DirectWriteCustomFont::Init(Microsoft::WRL::ComPtr <IDXGISwapChain4> swapChain){
	HRESULT result = S_OK;
	...
	UINT dpi = GetDpiForWindow(WinApp::GetInstance()->GetHwnd());
...
// options：テキストの整形
//====================================
HRESULT DirectWriteCustomFont::DrawString(std::string str, Vector2 pos, D2D1_DRAW_TEXT_OPTIONS options, bool shadow){
	HRESULT result = S_OK;
	...
	// テキストレイアウトを作成
	result = pDWriteFactory->CreateTextLayout(wstr.c_str(), (UINT32)wstr.size(), pTextFormat.Get(), TargetSize.width, TargetSize.height, pTextLayout.GetAddressOf());
	if (FAILED(result)) { return result; }
