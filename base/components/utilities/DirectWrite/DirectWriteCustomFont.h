...
namespace FontList{
	const std::wstring FontPath[] ={
		L"project/gamedata/resources/fonts/NikkyouSans-mLKax.ttf",
	};
}
...
public:
	...
	// 初期化関数
	HRESULT Init(Microsoft::WRL::ComPtr <IDXGISwapChain4> swapChain);
	...
	// pos：描画ポジション
	// options：テキストの整形
	HRESULT DrawString(std::string str, Vector2 pos, D2D1_DRAW_TEXT_OPTIONS options, bool shadow = false);
